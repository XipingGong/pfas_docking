...
while IFS= read -r pdb_file; do
   ...
   #        continue
    ...
    # Copy the vina.sh script
    echo "$ cp $scripts_dir/vina.sh $job_dir"
            cp $scripts_dir/vina.sh $job_dir

    echo "$ bash vina.sh > vina.out 2>&1"
            bash vina.sh > vina.out 2>&1

    ## Display the results, e.g., RMSD values
    echo "$ grep 'RMSD' vina.out"
            grep 'RMSD' vina.out
    
    grep 'Elapsed time' vina.out
